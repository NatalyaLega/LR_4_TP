...
//функция, для работы с очередью(а имеено, чтобы вызывать её в основном свиче)->тут прописаны основные операции
...
void work_with_queue(T* stptr, TT x)
{
	...
	try
	{
		cout << "Введите размер очереди(от 0 до 50):\n--> ";
		cin >> size;
		if (size <= 0)
			throw "Размер очереди не может быть нулевым или отрицательным!";
		if (size > 50)
			throw "Размер очереди больше 50 - это недопустимо!";
	}
	...
	while (exit == 1)
	{
		...
		cout << "3 - Проверить очередь на пустоту" << endl;
		...
		case 1:
			int elem;
			cout << "Введите элемент стека, который хотите добавить: ";
			cin >> elem;
			q = q + elem;
			//q.print();
...
int main()
{
	...
	int choice, choice_type, exit = 1, exit1 = 1; //для работы с меню
